...
const staticMenuPath = path.join(dataDir, 'menu.json');
...
// Menu per date
export async function getMenuForDate(dateKey) {
  if (hasPostgresEnv()) {
    const { pgGetMenuForDate, pgSaveMenuForDate } = await loadPg();
    const existing = await pgGetMenuForDate(dateKey);
    if (existing) return existing;
    // Fallback to bundled static menu (read-only)
    try {
      const raw = fs.readFileSync(staticMenuPath, 'utf-8');
      const map = JSON.parse(raw || '{}');
      const fromStatic = map[dateKey] || null;
      if (fromStatic) {
        await pgSaveMenuForDate(dateKey, fromStatic);
        return fromStatic;
      }
    } catch {}
    return null;
  }
  const data = readJSON(staticMenuPath) || {};
  return data[dateKey] || null;
}

export async function saveMenuForDate(dateKey, menu) {
  if (hasPostgresEnv()) {
    const { pgSaveMenuForDate } = await loadPg();
    return pgSaveMenuForDate(dateKey, menu);
  }
  const menuPath = path.join(dataDir, 'menu.json');
  try {
    const all = readJSON(menuPath) || {};
    const current = all[dateKey] || {};
    all[dateKey] = {
      breakfast: menu.breakfast !== undefined ? menu.breakfast : (current.breakfast ?? null),
      lunch: menu.lunch !== undefined ? menu.lunch : (current.lunch ?? null),
    };
    writeJSON(menuPath, all);
  } catch {
    // ignore if read-only
  }
}

...
export async function pgGetMenuForDate(dateKey) {
  const pool = getPool();
  const client = await pool.connect();
  try {
    await ensureSchema(client);
    const { rows } = await client.query('SELECT breakfast, lunch FROM menu_items WHERE menu_date = $1::date', [dateKey]);
    if (rows.length === 0) return null;
    return { breakfast: rows[0].breakfast || null, lunch: rows[0].lunch || null };
  } finally {
    client.release();
  }
}

export async function pgSaveMenuForDate(dateKey, menu) {
  const pool = getPool();
  const client = await pool.connect();
  try {
    await ensureSchema(client);
    await client.query(
      `INSERT INTO menu_items (menu_date, breakfast, lunch)
       VALUES ($1::date, $2, $3)
       ON CONFLICT (menu_date)
       DO UPDATE SET 
         breakfast = COALESCE(EXCLUDED.breakfast, menu_items.breakfast),
         lunch = COALESCE(EXCLUDED.lunch, menu_items.lunch),
         updated_at = now()`,
      [dateKey, menu.breakfast ?? null, menu.lunch ?? null]
    );
  } finally {
    client.release();
  }
